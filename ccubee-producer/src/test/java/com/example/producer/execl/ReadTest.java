...
import com.example.producer.execl.listener.ExcelNewListener;
import com.example.execl.vo.*;
import com.example.producer.execl.listener.DemoDataListener;
import com.example.producer.execl.listener.IndexOrNameDataListener;
import com.example.producer.execl.vo.*;
import com.example.producer.utils.TestFileUtil;
...
//        EasyExcel.read(fileName, DemoData.class, new DemoDataListener()).sheet().doRead();
        ...
        // 写法2：
        fileName = TestFileUtil.getPath() + "demo" + File.separator + "demo.xlsx";
        ExcelNewListener<T> listener = new ExcelNewListener<>();
        ExcelReader excelReader = EasyExcel.read(fileName, DemoData.class, listener).build();

        ReadSheet readSheet = EasyExcel.readSheet(0).build();
        excelReader.read(readSheet);
        // 这里千万别忘记关闭，读的时候会创建临时文件，到时磁盘会崩的
        listener.getRows().forEach(System.out::println);
        excelReader.finish();
    }
